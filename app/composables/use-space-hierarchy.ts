...
const isOrphanedRoom = (type: string | undefined) => type !== RoomType.Space && type !== RoomType.ElementVideo && type !== RoomType.UnstableCall

export function useSpaceHierarchy(spaceId: MaybeRefOrGetter<string | undefined>, enabled: MaybeRefOrGetter<boolean> = true) {
  ...
  const subspaceList = computed(() => Array.from(rooms.value.values()).filter(r => r.room_id !== spaceIdRef.value && r.room_type === RoomType.Space))

  const subspacesQueries = useQueries({
    queries: computed(() => subspaceList.value.map((space) => {
      ...
      if (localRoom) {
        ...
        for (const e of getStateEvents(localRoom, EventType.SpaceChild)) {
          ...
          if (!childId || !e.getContent().via?.length)
            continue
          ...
          if (!isOrphanedRoom(childRoom.getType()))
            count++
        }
      ...
      return {
        ...
        initialData: isDefined(localCount) ? { _localCount: localCount, rooms: [] } as IRoomHierarchyLocal : undefined,
        ...
        select: (data: unknown) => {
          ...
          if (isDefined(d._localCount))
            return d._localCount
